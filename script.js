...
const state = {
  ...
  apiToken: localStorage.getItem("bt_api_token") || null,
  apiOnline: false
};
...
function persist() {
  ...
  if (state.apiToken) localStorage.setItem("bt_api_token", state.apiToken);
}
...
async function api(path, options = {}) {
  const res = await fetch(`${API_BASE}${path}`, {
    headers: { "Content-Type": "application/json", ...(state.apiToken ? { Authorization: `Bearer ${state.apiToken}` } : {}), ...(options.headers || {}) },
    ...options
  });
...
async function syncMatchesFromApi() {
  ...
  try {
    const apiMatches = await api(`/matches`);
    const ids = new Set();
...
function onDecision(direction) {
  ...
  if (state.apiOnline && state.meUserId) {
    api("/swipes", { method: "POST", body: JSON.stringify({ toUserId: String(current.id), direction }) })
...
qs("onboardingForm").addEventListener("submit", async (e) => {
  ...
  if (state.apiOnline && !state.meUserId) {
    try {
      const user = await api("/auth/signup", { method: "POST", body: JSON.stringify({ email: `${state.me.name.replace(/\s+/g,"").toLowerCase()}@demo.local`, password: "pass1234" }) });
      state.meUserId = user.id;
      state.apiToken = user.token;
    } catch {}
  }
  if (state.apiOnline && state.meUserId) {
...
matchesList.addEventListener("click", (e) => {
  ...
  if (reportId) {
    ...
    if (state.apiOnline && state.meUserId) {
      api("/reports", { method: "POST", body: JSON.stringify({ targetUserId: String(reportId), reason: "user_reported" }) }).catch(() => {});
      api("/blocks", { method: "POST", body: JSON.stringify({ targetUserId: String(reportId) }) }).catch(() => {});
    }
    persist();
    renderMatches();
...
qs("chatForm").addEventListener("submit", (e) => {
  ...
  persist();
  if (state.apiOnline && state.meUserId) {
    api("/messages", { method: "POST", body: JSON.stringify({ matchId: state.activeChatId, text }) }).catch(() => {});
  }
