...
matchesList.addEventListener("click", (e) => {
  ...
  if (reportId) {
    ...
    if (state.apiOnline && state.meUserId) {
      api("/reports", { method: "POST", body: JSON.stringify({ fromUserId: state.meUserId, targetUserId: String(reportId), reason: "user_reported" }) }).catch(() => {});
      api("/blocks", { method: "POST", body: JSON.stringify({ fromUserId: state.meUserId, targetUserId: String(reportId) }) }).catch(() => {});
    }
    persist();
    renderMatches();
