...
import { api } from "./backend/src/clientApi.js";
...
async function onDecision(direction) {
  const pool = discoverPool();
  ...
  try {
    await api.swipe({ toUserId: current.id, direction: direction === "right" ? "RIGHT" : "LEFT" });
  } catch (err) {
    console.warn("swipe API sync failed", err);
  }
...
qs("onboardingForm").addEventListener("submit", async (e) => {
  ...
  try {
    if (!localStorage.getItem("bt_token")) {
      const email = `${String(state.me.name || 'user').toLowerCase().replace(/\s+/g, '.')}@businesstinder.local`;
      const auth = await api.register({ email, password: "changeme123", fullName: state.me.name || "User" });
      localStorage.setItem("bt_token", auth.token);
    }
    await api.saveProfile({
      role: state.me.role || "",
      bio: state.me.bio || "",
      interests: String(state.me.interests || "").split(",").map((x) => x.trim()).filter(Boolean),
      goals: [state.me.goal || ""],
      location: state.me.location || ""
    });
  } catch (err) {
    console.warn("backend sync failed, continuing in local prototype mode", err);
  }
